...
static void stab_listing(struct pico *p, char *listing) {
	...
	if(f == NULL) {
		if(!quiet_mode)
			fprintf(stderr, "Could not open listing file '%s'\n", listing);
		return;
	}
...
int main(int argc, char *argv[argc]) {
  ...
  if (srcfile == NULL) {
    help(argv[0]);
    fprintf(stderr, "No source file specified. Use -i<srcfile> to specify the source file.\n");
    return EXIT_FAILURE;
  } else {
	...
	if(assembler_run(&p) && !error_occurred) {
		result = EXIT_SUCCESS;
		output_flush(&p);
	}
